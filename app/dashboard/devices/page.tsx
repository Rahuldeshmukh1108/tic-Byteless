...
import { Plus, Settings, Trash2, Wifi, WifiOff, Thermometer, Droplets, Sun, Wind, Activity, AlertTriangle, Play, Square } from 'lucide-react'
...
function getRuntimeStatus(device: Device, liveData: LiveData | null | undefined): Device['status'] {
  if (!liveData) return device.status
  return Date.now() - new Date(liveData.timestamp).getTime() < 2 * 60 * 1000 ? 'online' : 'offline'
}

export default function DevicesPage() {
  const { user } = useAuth()
  const { devices, isLoading, error, removeDevice } = useDeviceContext()
  ...
  const handleDeviceConnected = (_deviceId: string, device: Device) => {
  ...
  const getStatusBg = (status: Device['status']) => {
    switch (status) {
      case 'online':
      ...
      default:
        return 'bg-gray-100 dark:bg-gray-500/20 border-gray-300 dark:border-gray-500/50'
    }
  }
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid gap-4">
          {[1, 2, 3].map((item) => (
            <div key={item} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/50 backdrop-blur-sm p-6 animate-pulse">
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
            Pair devices using the same hardware ID you set in the ESP32 sketch, such as `device001`.
          </p>
        </div>
        <button
      ...
      <div className="grid gap-6">
        {devices.map((device, index) => {
          ...
          const runtimeStatus = getRuntimeStatus(device, liveData)
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center smooth-transition ${getStatusBg(runtimeStatus)}`}>
                    {runtimeStatus === 'online' ? (
                      <Wifi size={24} className={getStatusColor(runtimeStatus)} />
                    ) : (
                      <WifiOff size={24} className={getStatusColor(runtimeStatus)} />
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-500">Last active: {formatLastSync(liveData?.timestamp)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    {runtimeStatus === 'online' ? 'Online' : runtimeStatus === 'offline' ? 'Offline' : 'Error'}
                  </span>
              ...
              {liveData && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <DataCard icon={<Thermometer size={16} className="text-red-500" />} label="Temperature" value={`${liveData.temperature.toFixed(1)} C`} />
                  <DataCard icon={<Droplets size={16} className="text-blue-500" />} label="TDS" value={`${liveData.tds} ppm`} />
                  <DataCard icon={<Sun size={16} className="text-yellow-500" />} label="Light" value={`${liveData.ldr}`} />
                  <DataCard icon={<Wind size={16} className="text-green-500" />} label="Humidity" value={`${liveData.humidity.toFixed(1)}%`} />
                </div>
              )}
              ...
              {liveData && (
                <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <StatusPill active={liveData.pump1Status} label="Pump 1" />
                    <StatusPill active={liveData.pump2Status} label="Pump 2" />
                    <StatusPill active={liveData.pump3Status} label="Pump 3" />
                    <StatusPill active={liveData.fanStatus} label="Fan" />
                    <StatusPill active={liveData.growLightStatus} label="Grow Light" />
                  </div>
                </div>
              )}

              {!liveData && runtimeStatus === 'online' && (
      ...
      {devices.length === 0 && !isLoading && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600 dark:text-slate-400 mb-2">Connect your first ESP32 device to start monitoring your hydroponic system.</p>
          <p className="text-xs text-slate-500 dark:text-slate-500 mb-6">
            The device ID in the app must match the `deviceID` string inside your ESP32 code.
          </p>
          <button
            onClick={() => setShowModal(true)}
      ...
      <DeviceConnectModal isOpen={showModal} onClose={() => setShowModal(false)} userId={user?.uid || ''} onDeviceConnected={handleDeviceConnected} />
...
function StatusPill({ active, label }: { active: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <div className={`w-3 h-3 rounded-full ${active ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
